...
export class SharedService {
     ...
     */
    openGenericDialog(component: any, data: any, size?: string, height?: string, directInject?: any[], disableClose?: boolean, panelClass?: string): Observable<any> {
        const dialogConfig = new MatDialogConfig();
     ...
     */
    findInvalidControlsRecursive(formToInvestigate: FormGroup | FormArray, scroll?: boolean): string[] {
        ...
        if (scroll) {
            let errorRef: HTMLElement = document.querySelector('[formControlName="' + invalidControls[0] + '"]');
            errorRef = errorRef ? errorRef : document.querySelector('[id="' + invalidControls[0] + '"]');
        }
    ...
    */
    muestraConfirmacion(tipo: string, elemento?: any, atributo?: string, visual?: string): Observable<any> {
        const dialogRef = this.dialog.open(ConfirmacionComponent, this.openDialog(35));
